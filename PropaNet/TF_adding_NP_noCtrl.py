...
def TF_adding_NP(DEGli, geneSet, TFli, TFrankFile, DEGnetFile, seed, coverNo=200, coverage=None):
    ...
    for iTF,TF in enumerate(TFli_rank):
        ...
        TGtmps.append(len(TGtmp))
        non_empty += 1 if len(TGtmp) != 0 else 0
    ...
    with open('TGtmps.txt', 'w') as f:
        f.write('\n'.join([str(t) for t in TGtmps]))
    print("len(TFli_rank)", len(TFli_rank))
    print("non_empty", non_empty)
    print("non_empty", non_empty)
    print("len(TF trimmed)", len(TF_trimmed))
    print()
    seed2weight = seed.loc[TF_trimmed,:].T.to_dict('records')[0]
...
#    parser.add_argument('-r',help='# repeats for IM')
    ...
    parser.add_argument('-c',default='1.0',type=float,help='coverage threshold')
    parser.add_argument('-coverNo',default='2000',type=float)
